...
const META = { "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", Accept: "text/x-userscript-meta,*/*", "User-Agent": CHROME };
...
check("Accept: text/x-userscript-meta,*/* (Violentmonkey/Tampermonkey frissítés-ellenőrzés) → check", classify(request(META)).kind, "check");
check("Accept: text/x-userscript-meta önmagában → check", classify(request({ Accept: "text/x-userscript-meta", "User-Agent": FIREFOX })).kind, "check");
check("Accept súlyozva, második helyen → check", classify(request({ Accept: "*/*;q=0.8, text/x-userscript-meta;q=0.9", "User-Agent": CHROME })).kind, "check");
check("Range-fejléc önmagában nem ellenőrzés (a Tampermonkey nem küld ilyet) → download", classify(request({ "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", Range: "bytes=0-4095", "User-Agent": CHROME })).kind, "download");
...
console.log("\n3) a letöltés útján: adatpont, majd az asset; ellenőrzésre csak a fejlécblokk");
const HEADER = "// ==UserScript==\n// @name           Neptun PowerUp! NG\n// @version        0.13.1\n// ==/UserScript==\n";
const SCRIPT = HEADER + "(() => {\n  /* 150 KB kód */\n})();\n";
const points = [];
let assetCalls = 0;
const assets = { fetch: async (req) => { assetCalls++; return new Response(req.method === "HEAD" ? null : SCRIPT, { status: 200, headers: { "Content-Type": "text/javascript", ETag: '"abc123"', "X-Method": req.method } }); } };
...
let res = await worker.fetch(request({ ...META, "User-Agent": EDGE }), env);
check("ellenőrzésre csak a fejlécblokk jön", await res.text(), HEADER);
check("a típusa text/x-userscript-meta", res.headers.get("Content-Type"), "text/x-userscript-meta; charset=utf-8");
check("saját ETag az asset ETag-jéből", res.headers.get("ETag"), 'W/"meta-abc123"');
check("Vary: Accept (ugyanaz az URL, mint a teljes fájlé)", res.headers.get("Vary"), "Accept");
check("nem cache-elhető revalidálás nélkül", res.headers.get("Cache-Control"), "public, max-age=0, must-revalidate");
check("egy adatpont íródott", points.length, 1);
check("index: a fajta", points[0].indexes[0], "check");
check("blobok: fajta, böngésző, ország, alak, státusz", points[0].blobs.join("|"), "check|Edge|HU|meta|200");
check("egy darab", points[0].doubles[0], 1);
check("nincs benne IP vagy UA", JSON.stringify(points[0]).includes("Mozilla"), false);
res = await worker.fetch(request({ ...META, "If-None-Match": 'W/"meta-abc123"' }), env);
check("ismert ETag-gel 304", res.status, 304);
check("304-nek nincs törzse", await res.text(), "");
check("a 304 is számít ellenőrzésnek, a státusszal", points[1].blobs.join("|"), "check|Chrome|HU|meta|304");
res = await worker.fetch(request(META, { method: "HEAD" }), env);
check("HEAD ellenőrzés: 200, törzs nélkül", res.status === 200 && (await res.text()) === "", true);
res = await worker.fetch(request({ "Sec-Fetch-Mode": "no-cors", "Sec-Fetch-Dest": "empty", "User-Agent": FIREFOX }), env);
check("teljes letöltésre a teljes fájl jön", await res.text(), SCRIPT);
check("a teljes fájl válaszán is Vary: Accept", res.headers.get("Vary"), "Accept");
check("letöltés adatpontja", points[3].blobs.join("|"), "download|Firefox|HU|full|200");
await worker.fetch(request({ "Sec-Fetch-Mode": "navigate", "Sec-Fetch-Dest": "document", "User-Agent": FIREFOX }, { cf: { country: "RO" } }), env);
check("telepítés-kattintás is számol", points[4].blobs.join("|"), "install|Firefox|RO|full|200");
res = await worker.fetch(request({ "User-Agent": CHROME }, { method: "HEAD" }), env);
check("HEAD letöltés: számol", points.length, 6);
...
check("POST: nem számol", points.length, 6);
...
check("ha a számlálás hibázik, az asset akkor is jön", await res.text(), SCRIPT);
const noHeader = { fetch: async () => new Response("nincs fejlécblokk", { status: 200, headers: { ETag: '"x"' } }) };
res = await worker.fetch(request(META), { ...env, ASSETS: noHeader });
check("ha nincs fejlécblokk, ellenőrzésre is a teljes tartalom megy", await res.text(), "nincs fejlécblokk");
const broken = { fetch: async (req) => { if (req.headers.get("Accept") === null) throw new Error("belső hiba"); return new Response(SCRIPT, { status: 200 }); } };
res = await worker.fetch(request(META), { ...env, ASSETS: broken });
check("ha a fejlécblokk-válasz hibázik, a teljes fájl jön", await res.text(), SCRIPT);
res = await worker.fetch(request(META), { ...env, ASSETS: { fetch: async () => new Response("nincs", { status: 404 }) } });
check("404 az assettől: 404 megy tovább", res.status, 404);
